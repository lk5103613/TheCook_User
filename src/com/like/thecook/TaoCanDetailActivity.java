...
public class TaoCanDetailActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDialog = new PopupWindow(popupView, LayoutParams.WRAP_CONTENT,
		...
		if (mPackId != -1) {
			...
			mDataFetcher.fetchTCDetail(mPackId, new Listener<JSONObject>() {
				...
				public void onResponse(JSONObject response) {
					showLoading(false);
					mDetail = GsonUtil.gson.fromJson(response.toString(),
							TCDetail.class);
					setupView(mDetail);
		...
		if (!TextUtils.isEmpty(mMsId)) {
			...
			mDataFetcher.fetchMSDetail(mMsId, new Listener<JSONObject>() {
				...
				public void onResponse(JSONObject response) {
					showLoading(false);
					Meishi ms = GsonUtil.gson.fromJson(response.toString(),
							Meishi.class);
					setupView(ms);
	...
	private void setupView(Meishi ms) {
		MyNetworkUtil
				...
				.getImageLoader()
				.get(APIS.BASE_URL + ms.avatar,
						ImageLoader.getImageListener(mTCImg, R.color.white,
								R.color.white));
	...
	private void setupView(TCDetail detail) {
		MyNetworkUtil
				.getInstance(mContext)
				.getImageLoader()
				.get(APIS.BASE_URL + detail.img_path,
						ImageLoader.getImageListener(mTCImg, R.color.white,
								R.color.white));
	...
	public void toCar(View view) {
		...
		final ShoppingCartEntity entity = new ShoppingCartEntity(
				mDetail.packname, Float.valueOf(mDetail.price), 1, 1,
				APIS.BASE_URL + mDetail.img_path);
