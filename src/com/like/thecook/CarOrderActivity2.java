...
import com.like.network.CommitOrderDao;
...
public class CarOrderActivity2 extends BaseActivity {
	...
	private List<ShoppingCartEntity> carEntites;
	 private ListView mListView;
	
	EditText  special_memoEt;
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							// /
							SharedPreferences preferences = getSharedPreferences(CONSTANTS.LOGIN_PROPERTIES, Context.MODE_PRIVATE);
							
							String uid = preferences.getString(
									"uid", "");
							Order po=new Order();
							String  dining_time=mMonth + " " + mWeek + " " + mTime;
							po.dining_time=dining_time;
							po.servicer_usercnt=servicer_usercnt;
							po.zidai_canju="0";
							String  special_memo=special_memoEt.getText().toString();
							if(special_memo!=null&&special_memo.length()>0){
								po.special_memo=special_memo;
							}else{
								po.special_memo="";
							}
							
							if(kitchen_size!=null&&kitchen_size.length()>0){
								po.kitchen_size=kitchen_size;
							}
							...
							if(mCarEntities!=null&&mCarEntities.size()>0){
								json=gson.toJson(mCarEntities);
							}
							
							po.json=json;
							
							CommitOrderDao.saveOrder(po);
...
//							po.setAddr_id(addrID + "");
//							po.setShip_zh(ship);
//							po.setUid(uid);
//							po.setAllWuLiuFee(allWuLiuFee);
//							CommitOrderDao.saveOrder(po);
//							
...
//							mHandler.sendEmptyMessage(1);
		...
		carEntites = mShoppingCartManager.getAll();
		mCarOrderAdapter = new CarOrderListAdapter(this, carEntites);
	...
	String    servicer_usercnt;
	String    zidai_canju="";//自带  1   否则  为空或者 0
		...
		if (mPayWindow == null) {
			...
			optionList.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View arg1,
					...
					servicer_usercnt = options.get(position);
					mLblPay.setText(servicer_usercnt);
	...
	public void selectArea(View v) {
		if (mAreaWindow == null) {
			...
			optionList.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View arg1,
						int position, long arg3) {
					kitchen_size = options.get(position);
					mArea.setText(kitchen_size);
